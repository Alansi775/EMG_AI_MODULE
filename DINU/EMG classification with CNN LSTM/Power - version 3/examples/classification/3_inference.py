...
# ---------------- Inference Thread ----------------
def inference_thread(update_text, emg_buffer, model, label_encoder, metadata, stop_flag):
...
def start_gui():
    ...
    inference_thread_ref = None  # To store thread reference
    ...
    def start_all():
        ...
        inference_thread_ref = threading.Thread(
            ...
            args=(update_text, emg_buffer, model, label_encoder, metadata, stop_flag),
            daemon=True
        )
    ...
    ttk.Button(btn_frame, text="🟥 Stop & Exit", command=stop_all).grid(row=0, column=1, padx=10)
